...
export const CAPABILITIES: CapabilitySpec[] = [
    ...
    { intent: 'web_search', summary: 'The task needs a public company, website, leadership, funding, revenue, competitor, or current public-web fact relevant to outreach — even when the user does not explicitly say “search”. The browser extension searches only after its one-time permission is approved. Extract a short search query into params.keywords; never claim you searched until browser results are returned.', sideEffect: false },
...
export function hardRules(): string[] {
    return [
        ...
        `Ask, do not assume. Never invent a missing business fact or silently choose between plausible meanings. If a needed detail is unknown or ambiguous — including the user's identity, company, offer, ICP, objective, CTA, audience, timeframe, or what “it”/“they” refers to — ask one concise clarifying question before researching, recommending, drafting, or taking an action. Use the authoritative profile and the conversation when they answer.`,
