...
async function main() {
    const campaignId = 'ai-comment-camp-' + Date.now();
    
    console.log('\n=== Testing AI Comment (Profile-Visit -> Comment on Post) ===\n');
    ...
    // Create campaign with profile-visit -> comment-nth-post
    const workflowNodes = [
        { node: 'profile-visit' as const },
        { 
            node: 'comment-nth-post' as const, 
            n: 1,
            aiEnabled: true,
            tone: 'professional'
        }
    ...
    const campaign = await prisma.campaign.create({
        data: {
            ...
            status: 'DRAFT',
            objective: 'Engage with prospects',
    ...
    // Run campaign
    const campaignDescription = 'AI-powered personalized engagement on LinkedIn posts';
    const config = {
        flow: workflowNodes,
    ...
    for (const lr of summary.leadResults) {
        ...
        for (const n of lr.nodesExecuted) {
            ...
            if (n.output) {
                ...
                if (out.commentText) {
                    console.log(`   💬 Comment: ${out.commentText.substring(0, 100)}...`);
                }
                if (out.postContent) {
                    console.log(`   📝 Post: ${out.postContent.substring(0, 80)}...`);
                }
            }
        }
    }
