const { chromium } = require('playwright-extra');
const stealth = require('puppeteer-extra-plugin-stealth')();
chromium.use(stealth);
const fs = require('fs');
const path = require('path');

async function debug() {
  const userDir = '/home/shiva/Documents/linkedin-camp/sessions/eef74901-4b70-44a8-89b9-437f6210d5ff';
  const cookies = JSON.parse(fs.readFileSync(path.join(userDir, 'cookies.json'), 'utf-8'));
  const fp = JSON.parse(fs.readFileSync(path.join(userDir, 'fingerprint.json'), 'utf-8'));
  
  const browser = await chromium.launch({ headless: true, args: ['--no-sandbox'] });
  const context = await browser.newContext({ userAgent: fp.userAgent, viewport: null, locale: 'en-IN', timezoneId: 'Asia/Kolkata' });
  await context.addCookies(cookies);
  const page = await context.newPage();
  page.setDefaultTimeout(15000);
  
  await page.goto('https://www.linkedin.com/feed/update/urn:li:activity:7433555515122647040/', { waitUntil: 'domcontentloaded' });
  await new Promise(r => setTimeout(r, 8000));
  
  // Scroll down to find comment section
  await page.mouse.wheel(0, 800);
  await new Promise(r => setTimeout(r, 2000));
  
  // Check all [role="textbox"] elements
  const textboxes = await page.evaluate(() => {
    const boxes = document.querySelectorAll('[role="textbox"]');
    const results = [];
    for (const box of boxes) {
      const rect = box.getBoundingClientRect();
      results.push({
        tag: box.tagName,
        classes: box.className?.substring(0, 100) || '',
        contenteditable: box.getAttribute('contenteditable'),
        ariaLabel: box.getAttribute('aria-label') || '',
        visible: rect.width > 0 && rect.height > 0,
        rect: { x: rect.x, y: rect.y, w: rect.width, h: rect.height }
      });
    }
    return results;
  });
  
  console.log('=== Textboxes ===');
  console.log(JSON.stringify(textboxes, null, 2));
  
  // Try clicking the first visible textbox
  if (textboxes.length > 0) {
    const first = textboxes.find(t => t.visible);
    if (first) {
      console.log('Clicking textbox at y=', first.rect.y);
      await page.mouse.click(first.rect.x + 50, first.rect.y + 20);
      await new Promise(r => setTimeout(r, 2000));
      
      await page.keyboard.type('test comment', { delay: 40 });
      await new Promise(r => setTimeout(r, 3000));
      
      const submitBtns = await page.evaluate(() => {
        const btns = document.querySelectorAll('button');
        const results = [];
        for (const btn of btns) {
          const text = btn.textContent?.trim() || '';
          const rect = btn.getBoundingClientRect();
          if ((text.includes('Comment') || text.includes('Post') || text.includes('Reply')) && rect.width > 0) {
            results.push({ text, classes: btn.className.substring(0, 100), disabled: btn.disabled, ariaLabel: btn.getAttribute('aria-label') || '' });
          }
        }
        return results;
      });
      
      console.log('=== Submit Buttons After Typing ===');
      console.log(JSON.stringify(submitBtns, null, 2));
    }
  }
  
  await page.screenshot({ path: '/tmp/comment-page2.png' });
  await browser.close();
}

debug().catch(e => console.error(e));
