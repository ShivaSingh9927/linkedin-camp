...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, disabled, children, ...props }, ref) => {
    ...
    return (
      <Comp className={cn(buttonVariants({ variant, size, className }), loading && "relative")} ref={ref}
        disabled={Comp === "button" ? (disabled || loading) : undefined} aria-busy={loading || undefined} {...props}>
        {Comp === Slot ? children : <>{loading && <Loader2 className="w-4 h-4 animate-spin" aria-hidden />}{children}</>}
      </Comp>
    )
  }
)
