...
async function downloadCSV() {
    ...
    const originalBannerHtml = els.statusBanner.innerHTML;
    const originalBannerClass = els.statusBanner.className;

    els.statusBanner.className = 'status-banner running';
    els.statusBanner.innerHTML = `<div class="spinner-small"></div><span id="ai-status">Extracting structured data...</span>`;
    ...
    try {
        ...
        for (let i = 0; i < s.leads.length; i++) {
            ...
            if (aiStatus) aiStatus.textContent = `Extracting: ${i+1} of ${s.leads.length}...`;

            if (l.info) {
                const structuredData = window.LLM.extractInfo(l.info);
        ...
        console.error('AutoConnect LLM error:', e);
        els.statusBanner.className = 'status-banner error';
        els.statusBanner.innerHTML = `<span>⚠ AI extraction failed: ${escapeHtml(e.message)} — generating CSV with raw data</span>`;
    }
    ...
    // Restore original banner after 3 seconds
    setTimeout(() => {
        els.statusBanner.className = originalBannerClass;
        els.statusBanner.innerHTML = originalBannerHtml;
    }, 3000);
}
...
async function exportToBackend() {
    ...
    // Run LLM enrichment before export
    const originalHtml = els.statusBanner.innerHTML;
    const originalClass = els.statusBanner.className;
    els.statusBanner.className = 'status-banner running';
    els.statusBanner.innerHTML = `<div class="spinner-small"></div><span id="ai-export-status">Extracting structured data...</span>`;

    let ranLLM = false;
    try {
        if (!window.LLM || !window.LLM.extractInfo) {
            console.warn('AutoConnect: extractor not loaded, exporting raw data');
        } else {
            for (let i = 0; i < s.leads.length; i++) {
                const l = s.leads[i];
                const el = document.getElementById('ai-export-status');
                if (el) el.textContent = `Extracting: ${i+1} of ${s.leads.length}...`;
                if (l.info) {
                    const structuredData = window.LLM.extractInfo(l.info);
                    l.company = structuredData.company || l.company;
                    l.education = structuredData.education || '';
        ...
        console.error('AutoConnect: enrichment failed (exporting raw):', e);
    }
...
// ─── Initialize ─────────────────────────────────────────────
let llmReady = true; // regex-only mode, always ready

function preLoadLLM() {
    // No-op: regex extractor needs no initialization
    const overlay = document.getElementById('llm-load-overlay');
...
(async () => {
    ...
    // Hide the load overlay (regex extractor needs no init)
    preLoadLLM().catch(e => console.error('Init error:', e));
    ...
    document.getElementById('batch-link-footer')?.addEventListener('click', async (e) => {
        e.preventDefault();
