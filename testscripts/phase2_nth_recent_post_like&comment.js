...
async function startUltraStable() {
  ...
  try {
    ...
    // Set this variable to choose which post to target (1 = Latest, 2 = 2nd Latest, etc.)
    const targetPostNumber = 2; 
    
    // Scroll multiple times to ensure the targeted post actually loads into the DOM
    for(let i = 0; i < targetPostNumber + 2; i++) {
        await page.mouse.wheel(0, 800);
        await wait(1500);
    }

    // We pass targetPostNumber down into the evaluate function context
    const postLink = await page.evaluate((targetNum) => {
      // Arrays are 0-indexed, so Post #1 is index 0
      const targetIndex = targetNum - 1;

      // METHOD 1: Universal data-urn wrapper (Highly Reliable)
      // Grab all matching wrappers into an array
      ...
      // Filter out comments and deduplicate links (a single post might have a link on the image and the text)
      for (let link of links) {
        if (!link.href.includes('?commentUrn=')) {
          const cleanLink = link.href.split('?')[0]; 
          if (!uniqueLinks.includes(cleanLink)) {
              uniqueLinks.push(cleanLink);
          }
        }
      }

      if (uniqueLinks.length > targetIndex) {
          return uniqueLinks[targetIndex];
      }

      return null;
    }, targetPostNumber); // Passing the variable here

    if (postLink) {
      result.postUrl = postLink;
      console.log(`🌐 Found post #${targetPostNumber}! Navigating to: ${postLink}`);
      ...
      try {
        ...
        console.log(`⚠️ Could not find post #${targetPostNumber}. The user might not have that many posts.`);
    }
...
//How to use it:
//To target the 3rd post: Run node phase2_post.js 3
//To target the latest post: Run node phase2_post.js 1
//If you forget the number: Run node phase2_post.js (The script will automatically default to 1).